...
var tabIndex =1;
// $('header').click();
$( "header" ).prepend( "<a class='adaSkip' href='#mainContent'>Skip Navigation</a>" );
$( "header *" ).each(function() {
	...
	if(this.tagName == 'LI'){
		$(this).has('SPAN').attr('tabindex', tabIndex);
		tabIndex++;
	}
});
$('.body_container section').first().attr('tabindex', '0').attr('id', 'mainContent');
function desktopNav() {
	var navItem = $('.site_links li');
	...
	$('.adaSkip').focusout(function(){$(this).css('top', '-60px')});
}
desktopNav();
...
function mobileNav() {
	...
	mainNavContainer = $('.main_nav_container');
	...
	mobileNavIcon.click(function(event) {
		...
		templateContainer.toggleClass('mobileNavOpen');
		mainNavContainer.toggleClass('open');
	});	
	...
	$(document).click(function(event) {
		...
		templateContainer.removeClass('mobileNavOpen');
		mainNavContainer.removeClass('open');
	});
...
/*--STICKY NAVIGATION--*/
	...
	// 		mainNavContainer = $('.main_nav_container'),
	// 		heroImageMod = $('.hero_image_mod');

	// 	if (scrollTop > 64) {
	// 		//mainNavContainer.css({'position': 'fixed', 'top': '0'});
	// 		//heroImageMod.css({'margin-top': '5.5rem'});
 //          mainNavContainer.css({'position': 'relative', 'top': 'auto'});
	// 		heroImageMod.css({'margin-top': '0px'});
	// 	}
	// 	else {
	// 		mainNavContainer.css({'position': 'relative', 'top': 'auto'});
	// 		heroImageMod.css({'margin-top': '0px'});
...
function videoScaling() {
	...
	function fullVideoOverlay(event) {
		var playButton = $('.play_btn'),
		fullScreenVideoContainer = $('.full_screen_video_container'),
		fullScreenVideoIframe = $('.full_screen_video_container iframe'),
		closeButton = $('.full_screen_video_container .close_btn'),
		videoUrl;
		...
		playButton.click(function(event) {
			...
			var videoURL = "https://content.jwplatform.com/players/"+videoID+"-gHcDUa67.html";
...
// Auto select
// IP LOCATION KII ROUTES TRAFIC THROUGH GEORGA
...
function setRegion(region_name){
	...
	$('#p_lt_WebPartZone2_Page_pageplaceholder_p_lt_WebPartZone2_Main_BizForm1_viewBiz_DropDownListControl_dropDownList option[value=' + selection + ']')
	.attr('selected',true);
}
